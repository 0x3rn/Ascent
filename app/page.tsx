...
import { shortenCoverLetter, generateCoverLetter } from "@/app/actions/resume-ai";
import {
  FileText,
  Download,
  ExternalLink,
...
function ResumeBuilderInner() {
  ...
  const handleCopy = async () => {
    if (!coverBody) return;
    try { await navigator.clipboard.writeText(coverBody); setCopied(true); setTimeout(() => setCopied(false), 2000); } catch {}
  };

  const handleDelete = () => {
    setCoverBody(""); setCoverTargetRole(""); setCoverCompanyName("");
    setCoverUserName(""); setCoverSkills([]);
  };

  const handleShorten = async () => {
    if (!coverBody) return;
    setShortening(true);
    try { const shortened = await shortenCoverLetter(coverBody); setCoverBody(shortened); }
    catch (err) { console.error("Shorten failed:", err); }
    finally { setShortening(false); }
  };

  const handleCoverGenerate = (
    body: string, targetRole: string, companyName: string,
    userName: string, skills: string[], useResumeData: boolean = false
  ) => {
    setCoverBody(body);
    ...
    setCoverUseResumeData(useResumeData);
    if (isMobile) setCoverView("preview");
  };

  const handleRegenerate = async () => {
    if (!coverTargetRole || !coverCompanyName || !coverUserName) return;
    setRegenerating(true);
    try {
      // Rebuild background if useResumeData was toggled on
      let bg = "";
      const body = await generateCoverLetter(coverUserName, coverTargetRole, coverCompanyName, coverSkills, bg);
      setCoverBody(body);
    } catch (err) {
  ...
  useEffect(() => {
    if (isPreviewTab && isMobile) { updatePreviewScale(); const t = setTimeout(updatePreviewScale, 100); window.addEventListener("resize", updatePreviewScale); return () => { clearTimeout(t); window.removeEventListener("resize", updatePreviewScale); }; }
  }, [isPreviewTab, isMobile, updatePreviewScale]);

  useEffect(() => {
    if (!isResume && coverView === "preview" && isMobile) { updateCoverPreviewScale(); const t = setTimeout(updateCoverPreviewScale, 100); window.addEventListener("resize", updateCoverPreviewScale); return () => { clearTimeout(t); window.removeEventListener("resize", updateCoverPreviewScale); }; }
  }, [isResume, coverView, isMobile, updateCoverPreviewScale]);

  if (!hasMounted) return null;

  const coverPreviewEl = (
    <CoverLetterPreview body={coverBody} targetRole={coverTargetRole} companyName={coverCompanyName} userName={coverUserName} />
  );

  return (
    <>
      <div ref={printContentRef} className="resume-print-container" style={{ visibility: "hidden", height: 0, overflow: "hidden" }} aria-hidden="true">
      ...
      <div className="flex flex-col md:flex-row h-dvh md:h-screen overflow-hidden">
        <aside className="print:hidden w-full md:w-[440px] md:min-w-[440px] border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex flex-col h-full">
          <header className="shrink-0 px-4 md:px-5 py-3 md:py-4 border-b border-zinc-100 dark:border-zinc-800/50 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 md:gap-2.5">
              <div className="h-7 w-7 md:h-8 md:w-8 rounded-lg bg-indigo-600 flex items-center justify-center shrink-0"><FileText className="h-3.5 w-3.5 md:h-4 md:w-4 text-white" /></div>
              <div><h1 className="text-xs md:text-sm font-semibold text-zinc-900 dark:text-zinc-100 leading-tight">Ascent</h1><p className="text-[10px] md:text-[11px] text-zinc-500 leading-tight">AI Career Toolkit</p></div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 md:p-5">
            {isResume ? (
              <>
                ...
                {isPreviewTab && isMobile && (<div className="flex justify-center pt-2"><div className="w-full shadow-lg bg-white"><div ref={previewWrapperRef} className="overflow-hidden w-full" style={{ height: Math.ceil(A4_HEIGHT_PX * previewScale) }}><div style={{ transform: `scale(${previewScale})`, transformOrigin: "top left", width: A4_WIDTH_PX }}><ResumePreview /></div></div></div></div>)}
              </>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-1.5 flex-wrap print:hidden">
                  ...
                  <Button onClick={handleDelete} disabled={!coverBody} size="sm" variant="outline" className="gap-1.5 text-[10px] h-7 text-red-600 hover:text-red-700 border-red-200 hover:border-red-300"><Trash2 className="h-3 w-3" />Delete</Button>
                </div>
                <div className="flex justify-center"><div className="w-full shadow-lg bg-white"><div ref={coverPreviewWrapperRef} className="overflow-hidden w-full" style={{ height: Math.ceil(A4_HEIGHT_PX * coverPreviewScale) }}><div style={{ transform: `scale(${coverPreviewScale})`, transformOrigin: "top left", width: A4_WIDTH_PX }}>{coverPreviewEl}</div></div></div></div>
              </div>
            )}
          </div>
          <footer className="shrink-0 px-4 md:px-5 py-2.5 md:py-3 border-t border-zinc-100 dark:border-zinc-800/50 flex items-center gap-3"><p className="text-[10px] md:text-[11px] text-zinc-400">Powered by DeepSeek AI</p><a href="https://github.com/0x3rn/Ascent" target="_blank" rel="noopener noreferrer" className="ml-auto text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"><ExternalLink className="h-3.5 w-3.5 md:h-4 md:w-4" /></a></footer>
        </aside>
        <main className="hidden md:flex flex-1 bg-zinc-100 dark:bg-zinc-900 overflow-auto items-start justify-center p-6 shrink-0">
          <div className="flex flex-col items-center gap-4">
            {isResume ? (
              <div className="origin-top shadow-2xl"><ResumePreview /></div>
            ) : (
              <>
                <div className="flex items-center gap-2 self-start print:hidden flex-wrap">
                  <Button onClick={handleCopy} disabled={!coverBody} size="sm" variant="outline" className="gap-1.5 text-xs h-7">{copied ? <Check className="h-3 w-3 text-green-600" /> : <Copy className="h-3 w-3" />}{copied ? "Copied!" : "Copy Text"}</Button>
                  <Button onClick={handleShorten} disabled={!coverBody || shortening} size="sm" variant="outline" className="gap-1.5 text-xs h-7">{shortening ? <Loader2 className="h-3 w-3 animate-spin" /> : <Scissors className="h-3 w-3" />}{shortening ? "Shortening..." : "Shorten"}</Button>
                  <Button onClick={handleRegenerate} disabled={!coverBody || regenerating} size="sm" variant="outline" className="gap-1.5 text-xs h-7">{regenerating ? <Loader2 className="h-3 w-3 animate-spin" /> : <RotateCw className="h-3 w-3" />}{regenerating ? "Regenerating..." : "Regenerate"}</Button>
                  <Button onClick={handleDelete} disabled={!coverBody} size="sm" variant="outline" className="gap-1.5 text-xs h-7 text-red-600 hover:text-red-700 border-red-200 hover:border-red-300"><Trash2 className="h-3 w-3" />Delete</Button>
                </div>
                <div className="origin-top shadow-2xl">{coverPreviewEl}</div>
              </>
            )}
          </div>
        </main>
...
export default function Home() {
  return (<ResumeProvider><ResumeBuilderInner /></ResumeProvider>);
}