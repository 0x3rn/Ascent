...
function ResumeBuilderInner() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: builderMode === "cover-letter" ? "cover-letter" : builderMode === "interview" ? "interview-prep" : "resume",
  ...
  const handleCoverGenerate = (body: string, tRole: string, cName: string, uName: string, sk: string[], ud: boolean = false) => {
    setCoverBody(body); setCoverTargetRole(tRole); setCoverCompanyName(cName); setCoverUserName(uName); setCoverSkills(sk); setCoverUseResumeData(ud); if (isMobile) setCoverView("preview");
  };
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row h-dvh md:h-screen overflow-hidden">
        <aside className="print:hidden w-full md:w-[440px] md:min-w-[440px] border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex flex-col h-full">
          <header className="shrink-0 px-4 md:px-5 py-3 md:py-4 border-b border-zinc-100 dark:border-zinc-800/50 flex items-center justify-between gap-3"><div className="flex items-center gap-2 md:gap-2.5"><div className="h-7 w-7 md:h-8 md:w-8 rounded-lg bg-indigo-600 flex items-center justify-center shrink-0"><FileText className="h-3.5 w-3.5 md:h-4 md:w-4 text-white" /></div><div><h1 className="text-xs md:text-sm font-semibold text-zinc-900 dark:text-zinc-100 leading-tight">Ascent</h1><p className="text-[10px] md:text-[11px] text-zinc-500 leading-tight">AI Career Toolkit</p></div></div><Button onClick={() => handlePrint()} size="sm" className="gap-1 md:gap-1.5 shrink-0 text-xs h-7 md:h-8 px-2.5 md:px-3"><Download className="h-3 w-3 md:h-3.5 md:w-3.5" /><span className="hidden sm:inline">Download PDF</span><span className="sm:hidden">PDF</span></Button></header>
        ...
        <main className="hidden md:flex flex-1 bg-zinc-100 dark:bg-zinc-900 overflow-auto items-start justify-center p-6 shrink-0">
          <div className="flex flex-col items-center gap-4">
            <div className="flex items-center gap-3 self-start print:hidden bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 px-3 py-2 shadow-sm"><div className="flex items-center gap-1.5"><span className="text-[10px] text-zinc-500 font-medium">Font:</span><select value={themeFont} onChange={e => setThemeFont(e.target.value)} className="text-[10px] border-none bg-transparent focus:ring-0">{FONTS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}</select></div><div className="w-px h-4 bg-zinc-200 dark:bg-zinc-600" /><div className="flex items-center gap-1.5"><span className="text-[10px] text-zinc-500 font-medium">Color:</span><select value={themeAccent} onChange={e => setThemeAccent(e.target.value)} className="text-[10px] border-none bg-transparent focus:ring-0">{COLORS.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div></div>
