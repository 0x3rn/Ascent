    ...
    <div className="w-[210mm] min-h-[297mm] bg-white text-zinc-900 shadow-2xl print:shadow-none print:w-auto print:h-auto print:min-h-0 print:m-0" style={{ fontFamily }}>
      <div className="px-[18mm] py-[16mm] print:px-[18mm] print:py-[16mm]">
        <header className="text-center mb-3 print:break-after-avoid">
          ...
          <div className="flex flex-row flex-wrap items-center justify-center gap-x-4 gap-y-0.5 mt-1 text-[9pt] text-zinc-500">
        ...
        {experience.length > 0 && (
          <section className="mb-2.5 print:pt-[10mm] print:break-inside-avoid">
            ...
            {experience.map((exp) => (<div key={exp.id} className="mb-2 print:break-inside-avoid"><div className="flex justify-between items-baseline flex-wrap gap-1"><div><h3 className="text-[11.5pt] font-semibold leading-snug text-zinc-900">{exp.role || "Role"}{exp.company && (<span className="font-normal text-zinc-600"> | {exp.company}</span>)}</h3></div><div className="text-[9pt] text-zinc-500 whitespace-nowrap">{exp.startDate && <span>{exp.startDate}</span>}{exp.startDate && exp.endDate && <span> &ndash; </span>}{exp.endDate && <span>{exp.endDate}</span>}{exp.location && <span className="ml-1">{exp.location}</span>}</div></div>{exp.bullets && (<div className="mt-0.5 ml-0"><BulletList text={exp.bullets} /></div>)}</div>))}
          </section>
        )}
        {projects.length > 0 && (
          <section className="mb-2.5 print:pt-[10mm] print:break-inside-avoid">
            ...
            {projects.map((proj) => (<div key={proj.id} className="mb-2 print:break-inside-avoid"><div className="flex justify-between items-baseline flex-wrap gap-1"><h3 className="text-[11.5pt] font-semibold leading-snug text-zinc-900">{proj.name || "Project Name"}</h3>{proj.link && <span className="text-[9pt] text-zinc-500 whitespace-nowrap">{proj.link}</span>}</div>{proj.skills && <p className="text-[9pt] italic text-zinc-600 mt-0.5">{proj.skills}</p>}{proj.bullets && (<div className="mt-0.5 ml-0"><BulletList text={proj.bullets} /></div>)}</div>))}
          </section>
        )}
        {education.length > 0 && (
          <section className="mb-2.5 print:pt-[10mm] print:break-inside-avoid">
